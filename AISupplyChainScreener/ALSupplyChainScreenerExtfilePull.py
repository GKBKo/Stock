...
# ── 티커 파일 로드 ─────────────────────────────────────────────────────────────
# 파일 위치: <이 스크립트가 있는 폴더>/tickers/tickers_AI.txt
#
# [파일 형식 예시] ── 두 가지 모두 지원 ──
#   티커만 적는 경우:          티커,회사명 같이 적는 경우:
#   NVDA                       NVDA,NVIDIA
#   AAPL                       AAPL,Apple Inc
#   MSFT                       MSFT,Microsoft
#
# - 빈 줄, # 로 시작하는 줄은 무시됩니다.
# - 회사명을 생략하면 yfinance의 shortName을 자동으로 가져옵니다.

def load_tickers(filename="tickers_AI.txt") -> list[tuple[str, str]]:
    base_dir   = os.path.dirname(os.path.abspath(__file__))
    ticker_dir = os.path.join(base_dir, "tickers")
    file_path  = os.path.join(ticker_dir, filename)

    if not os.path.exists(file_path):
        raise FileNotFoundError(
            f"\n❌ 티커 파일을 찾을 수 없습니다: {file_path}\n"
            f"   아래 경로에 파일을 만들어 주세요:\n"
            f"   {file_path}\n\n"
            f"   [파일 형식]\n"
            f"   # 주석은 # 으로 시작\n"
            f"   NVDA,NVIDIA          <- 티커,회사명\n"
            f"   AAPL                 <- 회사명 생략 시 자동 조회\n"
        )
    ...
    with open(file_path, encoding="utf-8") as f:
        for line in f:
            line = line.strip()
            if not line or line.startswith("#"):   # 빈 줄·주석 건너뜀
                continue
            parts = [p.strip() for p in line.split(",", maxsplit=1)]
            sym  = parts[0].upper()
            name = parts[1] if len(parts) == 2 else ""   # 회사명 없으면 빈 문자열
            tickers.append((sym, name))

    if not tickers:
        raise ValueError(f"❌ 티커 파일이 비어 있습니다: {file_path}")

    print(f"📂 티커 파일 로드 완료: {file_path}  ({len(tickers)}개)")
    return tickers

# ── 데이터 수집 ────────────────────────────────────────────────────────────────
def fetch_data(tickers):
    symbols = [t[0] for t in tickers]
    # 회사명이 비어있으면 yfinance shortName 으로 채움
    name_map = {}
    for sym, name in tickers:
        if name:
            ...
            try:
                ...
                name_map[sym] = info.get("shortName") or info.get("longName") or sym
            except Exception:
                name_map[sym] = sym
...
def fmt_pct_heatmap(v, all_vals):
    """수익률 크기에 비례해 녹색/빨간 배경 농도를 적용한 td 셀 반환"""
    if v is None or (isinstance(v, float) and np.isnan(v)):
    ...
    # 전체 분포에서 0~100% 사이 강도 계산 (절댓값 기준)
    abs_vals = [abs(x) for x in all_vals if x is not None and not np.isnan(x)]
    max_abs  = max(abs_vals) if abs_vals else 1
    intensity = min(abs(v) / max_abs, 1.0)   # 0.0 ~ 1.0

    if v >= 0:
        # 녹색: 투명(0%) → 진한 녹색(100%)
        alpha = 0.08 + intensity * 0.72      # 0.08 ~ 0.80
        bg    = f"rgba(34,197,94,{alpha:.2f})"
        fg    = "#d4d8e8" if intensity < 0.5 else "#0d0f14"
    else:
        # 빨간색: 투명(0%) → 진한 빨간(100%)
...
def build_html(df):
    ...
    # 히트맵용 전체 분포값 (NaN 제외)
    all_ytd = [v for v in df['pct_ytd'].tolist() if v is not None and not np.isnan(v)]
    all_1y  = [v for v in df['pct_1y'].tolist()  if v is not None and not np.isnan(v)]
    ...
    for i, r in df.iterrows():
        ...
        <tr>
          ...
          {fmt_pct_heatmap(r['pct_ytd'], all_ytd)}
          <td class="spark">{spark_svg(r['spark'])}</td>
          {fmt_pct_heatmap(r['pct_1y'], all_1y)}
...
def main():
    ...
    tickers = load_tickers()          # tickers/tickers_AI.txt 에서 로드
